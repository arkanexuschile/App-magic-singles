...
## Project-specific setup (price sync)

Set these environment variables:

- `EXTERNAL_PRICES_API_TIMEOUT_MS` (optional): Scryfall request timeout in milliseconds, default `15000`
- `EXCHANGE_RATE_CACHE_TTL_MS` (optional): USD/CLP exchange rate cache TTL in milliseconds, default `86400000` (24h)
- `EXCHANGE_RATE_CACHE_MAX_STALE_MS` (optional): maximum staleness allowed when both FX providers fail, default `172800000` (48h)
- `JUSTTCG_BY_EXTERNAL_ID_URL_TEMPLATE` (required for `justtcg`): endpoint template with `{id}` placeholder (example: `https://api.example.com/cards/byExternalID/{id}`)
- `JUSTTCG_API_TOKEN` (optional): Bearer token for JustTCG API
- `CARDKINGDOM_PRICELIST_URL` (optional): Card Kingdom pricelist endpoint, default `https://api.cardkingdom.com/api/pricelist`
- `CARDKINGDOM_PRICELIST_FALLBACK_URL` (optional): fallback endpoint if primary fails, default `https://api.cardkingdom.com/api/v2/pricelist`
- `CARDKINGDOM_SYNC_INTERVAL_MS` (optional): how often Card Kingdom prices are refreshed into DB, default `3600000` (1h)
- `CARDKINGDOM_PRICELIST_CACHE_TTL_MS` (optional, deprecated alias): same behavior as `CARDKINGDOM_SYNC_INTERVAL_MS`
- `CARDKINGDOM_DB_MEMORY_CACHE_TTL_MS` (optional): in-process cache TTL for DB reads, default `300000` (5m)
- `CARDKINGDOM_REFRESH_LOCK_LEASE_MS` (optional): distributed lock lease to avoid duplicate refresh in multi-instance deployments, default `300000` (5m)
- `APP_SUPPORT_EMAIL` (required for review): public support email shown in privacy/terms/support pages

Run database migration before first execution:
...
- `/app/price-sync`:
  - Daily schedule (UTC)
  - Search mode (`sku`, `title`, `metafield`)
  - Price source (`scryfall`, `justtcg` byExternalID, `mtgjson`, `cardkingdom` pricelist)
  - Target currency selector (`USD` or `CLP`)
  - Optional mode to use merchant-owned Scryfall ID metafield
  - Previous price tracking in `custom.previous_price`
  - Optional image sync
  - First-run Scryfall ID metafield initialization
  - Card Kingdom prices are refreshed into local DB cache every hour

### Metafield usage

When search mode is set to `metafield`, the app reads a metafield from each product variant and uses that value to resolve the card in Scryfall.

- Configure in the panel:
  - `Search metafield namespace`
  - `Search metafield key`
- Expected value in that metafield:
  - Scryfall card ID (example: `c2b9d0f1-5a2d-4b8b-9d2d-0f2a3b4c5d6e`)

Internal metadata field used by the app:

- On first metadata initialization run, the app stores Scryfall ID in:
  - namespace: `custom`
  - key: `scryfall_meta`
- This internal field is not editable in the sync panel.

Optional merchant-owned Scryfall ID mode:

- Enable `Use my own Scryfall ID metafield` in `/app/price-sync`
- Configure:
  - `Custom Scryfall ID namespace`
  - `Custom Scryfall ID key`
- In this mode, the app uses your field for `scryfall`, `justtcg`, and `cardkingdom` price lookup and does not write `custom.scryfall_meta`.

### Currency conversion and previous price metafield

When target currency is `CLP`, the app converts Scryfall USD values to Chilean pesos using:

- `https://mindicador.cl/api/dolar` (primary)
- `https://open.er-api.com/v6/latest/USD` (fallback)

The exchange rate is cached in DB and refreshed only when expired. If both providers fail, the app can temporarily reuse a stale cached rate (up to `EXCHANGE_RATE_CACHE_MAX_STALE_MS`).

On each successful price change, the app stores the previous variant price in:
...
When search mode is `sku`, SKU parsing uses:

- `[first 3 chars set code][collector_number][foil/nonfoil or empty][language]`
- Examples:
  - `neo123foilen`
  - `khm45nfen`
  - `m2110es`

Compliance and review readiness already included in this project:
...
For copy/paste App Store listing text and reviewer instructions, see:

- `docs/shopify-review-submission.md`
